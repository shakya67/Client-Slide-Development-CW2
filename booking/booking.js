...
$(function () {
  ...
  if (!car) {
    // fallback
    car = { name: 'No car selected', img: '../img/car-placeholder.jpg', desc: 'Select a car from the shop', price: 0 };
  }

  // jQuery Debugging Solution - Add this block
  ...
  const $img = $('<img>', {
    ...
    console.log('✅ Successfully loaded:', car.img);
  });
  ...
  $track.empty();
  const img = $('<img>').attr('src', car.img || '../img/car-placeholder.jpg').attr('alt', car.name + ' img');
  $track.append(img);
